...
namespace HTLBIWebApp2012
{
	...
	public class InqFieldInfoMDX : InqMDX
	{
		...
		public override string ToMDX()
		{
			...
			{
				return string.Format("[{0}].[{1}].[{1}]", this.TblName, this.ColName);
			}
			return string.Format("ORDER([{0}].[{1}].[{1}],[{0}].[{1}].CURRENTMEMBER.NAME,{2})", this.TblName, this.ColName, this.OrderName);
		}
		/// <summary>
		/// Trả về tên field theo cú pháp của MDX ([TableName].[ColumnName].CHILDREN). 
		...
		{
			if (string.IsNullOrEmpty(filterSetName))
			{
				return this.ToMDX();
			}
			if (!this.IsValid())
