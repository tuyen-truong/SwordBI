...
//===========
    ...
    */
	...
	if (dom)
	{
		...
		document.write ("<div onclick='bShow=true' id='calendar' class='div-style'><table width='" +((showWeekNumber==1)?250:220)+"' class='table-style'><tr class='title-background-style' ><td class='tdCalendar'><table width='"+((showWeekNumber==1)?248:218)+"' class='tableCalendar'><tr><td class='title-style'><B><span id='caption'></span></B></td><td align=right><a href='javascript:hideCalendar()'><IMG SRC='"+imgDir+"close.gif' WIDTH='15' HEIGHT='13' BORDER='0' ALT='Close the Calendar'></a></td></tr></table></td></tr><tr><td class='body-style'><span id='contentCalendar'></span></td></tr>")
	...
	function hideCalendar()	{
		if(crossobj){
			...
			//PhiHuynh modified 2007/02/08
			if (navigator.appVersion.substr(22,3) < "7.0"){
				showElement( 'SELECT' );
				showElement( 'APPLET' );
			}
			//showElement( 'SELECT' );
	...
	function closeCalendar() {
		...
		if(ExtrCtr)
			ExtrCtr.value =	constructDate(dateSelected,monthSelected,yearSelected)
	}
	...
	function popUpMonth() {
		...
		crossMonthObj.left = parseInt(crossobj.left, 10) + 50
		crossMonthObj.top =	parseInt(crossobj.top, 10) + 26
	}
	...
	function popUpYear() {
		...
		crossYearObj.top = parseInt(crossobj.top, 10) +	26
	}
		...
		for	( datePointer=1; datePointer<=numDaysInMonth; datePointer++ )
		{
			...
			if ((dayPointer+startAt) % 7 == startAt) { 
				...
				if ((showWeekNumber==1)&&(datePointer<numDaysInMonth))
				{
					...
					crossHeight = crossHeight + 18
				}				
			}
		}

		document.getElementById("contentCalendar").innerHTML   = sHTML
	...
	function popUpCalendar(ctl,	ctl2, format, OtherFunctions, leftFlg, topFlg) {//PhiHuynh added leftFlg = 'true' and topFlg = 'true'
		leftFlg = leftFlg || "";
		topFlg = topFlg || "";
		var leftpos = 0;
		var toppos = 0;
		...
		if (bPageLoaded)
		{
			if ( crossobj.visibility ==	"hidden" ) {
                ...
                //Start, Duc modified 20100201
				//crossobj.left =	fixedX==-1 ? ctl.offsetLeft	+ leftpos :	fixedX
				crossobj.right = 0      // very good
				//End, Duc modified 20100201 
				crossobj.top = fixedY==-1 ?	ctl.offsetTop +	toppos + ctl.offsetHeight +	2 :	fixedY
				constructCalendar (1, monthSelected, yearSelected);
				
				//Start, Duc del 20100201
//				//PhiHuynh added 2007/03/08
//				if (leftFlg == 'true')
//					crossobj.left = crossobj.pixelLeft - 27*8 - 6;
//				if (topFlg == 'true')
//					crossobj.top = crossobj.pixelTop - crossHeight;
//				//--end
                //End, Duc del 20100201
		...
		{
			init()
			popUpCalendar(ctl,	ctl2, format, OtherFunctions, leftFlg, topFlg)
		}
	}
	...
	//window.onload=init()
	//==========================================//
	//==== lettpos , toppos , from  image=======//
	//=== Add by Bang Nguyen ===================//
	function CustompopUpCalendar( leftpos, toppos, ctl,	ctl2, format, OtherFunctions) {
		oFunctions = OtherFunctions
		if (bPageLoaded)
		{
			if ( crossobj.visibility ==	"hidden" ) {
		...
		{
			init()
			CustompopUpCalendar(leftpos ,toppos , ctl,	ctl2, format, OtherFunctions)
		}
	}	
